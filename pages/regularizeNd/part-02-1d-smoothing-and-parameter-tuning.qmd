...
The script uses a small 1D dataset from the original educational blog progression and compares coarse and fine grids under the same smoothness value.
...
The example comments note that extremely tiny values can make the system ill-conditioned, which is exactly why log-step sweeps are safer than random tweaking.
...
5. Carry the candidate value into Part 3 for interpolation and solver comparisons.

## Common Mistakes

1. Changing smoothness, interpolation, and solver all at once.
2. Using linear-scale sweeps such as `0.001, 0.002, 0.003` before understanding order-of-magnitude behavior.
