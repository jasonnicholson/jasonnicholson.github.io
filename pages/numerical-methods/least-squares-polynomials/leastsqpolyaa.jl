# leastsqpolyaa.jl
# Least Squares Polynomials animation: degree-2 fit construction
# Produces: leastsqpolyaa.gif
...
coeffs = [4.67425137, 0.52860951, -0.07563441]
...
anim = @animate for k in 0:2
    ...
    plot(size = (680, 440), xlims = xlims, ylims = ylims,
         xlabel = "x", ylabel = "y",
         title = "Least Squares Polynomial Fit: Degree 2 (terms through x^$(k))",
         legend = :bottomright, grid = true, framestyle = :box,
         background_color = :white)

    scatter!(x_data, y_data; color = :royalblue, ms = 5, label = "data points")
    plot!(xplot, y_full; color = :gray45, lw = 2, ls = :dash, label = "final p2(x)")
    plot!(xplot, y_partial; color = :deepskyblue3, lw = 2.5, label = "partial polynomial")
...
gif(anim, joinpath(@__DIR__, "leastsqpolyaa.gif"); fps = 1)
println("Saved: leastsqpolyaa.gif")
