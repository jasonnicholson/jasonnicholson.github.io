# leastsqpolyad.jl
# Least Squares Polynomials animation: degree-5 fit construction
# Produces: leastsqpolyad.gif
...
coeffs = [4.63232375, 0.50885354, -0.06062594, -0.00073781, -0.00065051, 0.00011667]
...
anim = @animate for k in 0:5
    ...
    plot(size = (680, 440), xlims = xlims, ylims = ylims,
         xlabel = "x", ylabel = "y",
         title = "Least Squares Polynomial Fit: Degree 5 (terms through x^$(k))",
         legend = :bottomright, grid = true, framestyle = :box,
         background_color = :white)

    scatter!(x_data, y_data; color = :royalblue, ms = 5, label = "data points")
    plot!(xplot, y_full; color = :gray45, lw = 2, ls = :dash, label = "final p5(x)")
    plot!(xplot, y_partial; color = :mediumvioletred, lw = 2.5, label = "partial polynomial")
...
gif(anim, joinpath(@__DIR__, "leastsqpolyad.gif"); fps = 1)
println("Saved: leastsqpolyad.gif")
