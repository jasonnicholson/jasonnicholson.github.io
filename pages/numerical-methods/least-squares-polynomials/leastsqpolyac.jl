# leastsqpolyac.jl
# Least Squares Polynomials animation: degree-4 fit construction
# Produces: leastsqpolyac.gif
...
coeffs = [4.62862816, 0.49668723, -0.05823430, 0.00201395, -0.00080138]
...
anim = @animate for k in 0:4
    ...
    plot(size = (680, 440), xlims = xlims, ylims = ylims,
         xlabel = "x", ylabel = "y",
         title = "Least Squares Polynomial Fit: Degree 4 (terms through x^$(k))",
         legend = :bottomright, grid = true, framestyle = :box,
         background_color = :white)

    scatter!(x_data, y_data; color = :royalblue, ms = 5, label = "data points")
    plot!(xplot, y_full; color = :gray45, lw = 2, ls = :dash, label = "final p4(x)")
    plot!(xplot, y_partial; color = :orangered3, lw = 2.5, label = "partial polynomial")
...
gif(anim, joinpath(@__DIR__, "leastsqpolyac.gif"); fps = 1)
println("Saved: leastsqpolyac.gif")
