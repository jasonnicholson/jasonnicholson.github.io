# aitkencd.jl
# Aitken/Steffensen animation: Examples 3 & 4 merged (subplot)
# f(x) = (x^2 - 2)*sin(x^2 - 2),  double root p = sqrt(2),  x0 = 1.0
...
anim = @animate for frame in 0:n_total
    ...
    p1 = plot(size=(680, 420), xlims=xlims_curve, ylims=ylims_curve,
              xlabel="x", ylabel="y",
              title="Newton (blue) vs Steffensen (red): f(x) = (x²−2)sin(x²−2)  (p = √2)",
...
gif(anim, joinpath(@__DIR__, "aitkencd.gif"); fps=2)
println("Saved: aitkencd.gif")
