...
<p>$$D = \left( \frac{\partial^{2}F}{\partial
...
search is</p>
<ul>
<li><p>$\frac{\partial^{2}F}{\partial
x^{2}}$ is positive definite. Said another way, <span
...
iteration in the nonlinear system case. As far as I know, $\frac{\partial\ F}{\partial x}$ needs to be
invertible, thus nonsingular. I think that is the only requirement.
However, we would like to find a way for it to still work if
$\frac{\partial\ F}{\partial x}$ is sometimes
...
![Graph of line search](image1.png)
...
![Line search surface (view 1)](image2.png)

![Line search surface (view 2)](image3.png)

If we plot $x_{2}$ vs. $\alpha$ we get:

![Plot of $x_{2}$ vs $\alpha$](image4.png)
